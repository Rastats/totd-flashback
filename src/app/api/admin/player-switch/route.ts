...
export async function POST(request: Request) {
    try {
        ...
        // If just clearing active (for force switch), set active to null
        // The waiting player will become active automatically via next plugin sync
        if (new_active_player === undefined && new_waiting_player === undefined && current) {
            update.active_player = null;
            // waiting_player stays as-is, will become active on next sync
        }
